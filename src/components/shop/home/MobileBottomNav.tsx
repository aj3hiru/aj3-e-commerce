...
export function MobileBottomNav({ loggedIn }: { loggedIn: boolean }) {
  ...
  return (
    <nav aria-label="Quick navigation" className="fixed inset-x-0 bottom-0 z-[900] grid h-[58px] grid-cols-5 bg-white shadow-[0_-1px_6px_rgba(0,0,0,0.08)] shop:hidden">
      <Link href="/shop" aria-current="page" className={`${item} font-medium !text-[#353543]`}>
      ...
      <Link href="/shop/wishlist" className={item}><Heart className={icon} strokeWidth={1.5} />Wishlist</Link>
      ...
      <Link href={loggedIn ? "/shop/account" : "/shop/login"} className={item}><UserRound className={icon} strokeWidth={1.5} />{loggedIn ? "Account" : "Login"}</Link>
    </nav>
  );
}
