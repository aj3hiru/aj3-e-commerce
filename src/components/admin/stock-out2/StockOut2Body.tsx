                    ...
                    rows.map((p) => {
                      ...
                      return (
                        <tr key={p.id} style={{ height: ROW_H }} className={cn(selected.has(p.id) ? "bg-blue-50/60" : "odd:bg-[#f2f2f2] even:bg-white", isBusy && "opacity-60")}>
                          ...
                          {show("so2-c-status") && (
                            <td className={td}>
                              <PillMenu on={p.status === "active"} busy={isBusy} labels={["Active", "Inactive"]} name={`Status of ${p.name}`}
                                onChange={(on) => setStatus([p.id], on ? "active" : "inactive")} />
                            </td>
                          )}
                          {show("so2-c-stock") && (
                            <td className={td}>
                              <button type="button" disabled={isBusy} onClick={(e) => toggleEditor(p.id, e.currentTarget)} aria-haspopup="dialog" aria-expanded={editing}
                                aria-label={`Update stock for ${p.name}`} title="Click to update stock"
                                className={cn("inline-flex h-9 max-w-full items-center gap-2 rounded-[0.25rem] bg-[#dc3545] px-3 text-[13px] font-semibold text-white shadow-sm transition-colors hover:bg-[#bb2d3b] disabled:cursor-wait", editing && "ring-2 ring-[#dc3545]/30 ring-offset-1")}>
                                {isBusy ? <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin" /> : null}
                          ...
                          {show("so2-c-actions") && (
                            <td className={td}>
                              <div className="flex items-center gap-2">
                                <button type="button" disabled={isBusy} onClick={(e) => toggleEditor(p.id, e.currentTarget)} aria-label={`Restock ${p.name}`} title="Restock"
                                  className="flex h-10 w-10 items-center justify-center rounded-[0.375rem] bg-[#3d8b5f] text-white shadow-sm hover:bg-[#33774f] disabled:opacity-50">
                                  <PackagePlus className="h-4 w-4" />
                                </button>
                                <Link href={EDIT_HREF(p.id)} aria-label={`Edit ${p.name}`} title="Edit product"
...
/** The green "Active ▾" / grey "Inactive ▾" button from the current page; menu portalled so the table can't clip it. */
...
function Menu({ trigger, items, disabled }: { trigger: (open: boolean) => React.ReactNode; disabled?: boolean; items: { label: string; onClick: () => void; danger?: boolean }[] }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" disabled={disabled} aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen((o) => !o)} title={disabled ? "Select products first" : undefined} className="block disabled:cursor-not-allowed">
        {trigger(open)}
      </button>
      {open && (
