...
const RETURN_TARGETS = new Map<string, { href: string; label: string }>([
  ["stock-out-products2", { href: "/admin/ecommerce/stock-out-products", label: "Back to Stock Out" }],
]);
...
/**
 * /admin/ecommerce/products/add — a trial redesign of Add Product that also
 ...
 *   src/app/admin/ecommerce/products/add/
...
export default async function AddProduct2Page({ searchParams }: AddProduct2PageProps) {
  ...
  const back = (fromRaw ? RETURN_TARGETS.get(fromRaw) : undefined) ?? { href: "/admin/ecommerce/products", label: "Back to Products" };
        ...
        {notFound ? (
          <div className="flex flex-col items-center gap-3 rounded-xl border border-admin-gray-200 bg-white px-6 py-16 text-center shadow-sm">
            ...
            <div className="mt-2 flex gap-2">
              <Link href="/admin/ecommerce/products" className="flex h-10 items-center rounded-[0.5rem] border border-[#e5e7eb] bg-white px-4 text-sm font-medium text-[#374151] hover:bg-[#f9fafb]">All Products</Link>
              <Link href="/admin/ecommerce/products/add" className="flex h-10 items-center rounded-[0.5rem] bg-admin-primary px-4 text-sm font-semibold text-white hover:bg-admin-primary-dark">Add a new product</Link>
            </div>
          </div>
        ) : (
          <AddProduct2Form
