...
export function DisplayOptionsShell({ prefKey, groups, standalone = [], extraActions, children, ...shell }: Omit<React.ComponentProps<typeof AdminShell>, "headerActions"> & {
  prefKey: string; groups: readonly WidgetGroup[]; standalone?: readonly { key: string; label: string }[]; extraActions?: React.ReactNode;
}) {
  return (
    <DashboardWidgetPrefsProvider prefKey={prefKey} groups={groups} standalone={standalone}>
